...
public class PrintFormat
{
    ...
    public PrintFormat(String format, String timeFormat)
    {
        ...
        for(String spl : timeFormat.split(":"))
        {
            ...
            else if(spl.contains("q")) { timeOrganizer[1] = spl.length(); }
            else if(spl.contains("M")) { timeOrganizer[2] = spl.length(); }
            else if(spl.contains("s")) { timeOrganizer[3] = spl.length(); }
            else if(spl.contains("m")) { timeOrganizer[4] = spl.length(); }
            else if(spl.contains("h")) { timeOrganizer[5] = spl.length(); }
            else if(spl.contains("d")) { timeOrganizer[6] = spl.length(); }
        }
        timer = new Timer(timeOrganizer[0] > 0 || timeOrganizer[1] > 0);
    }

    public String getPrintString(String type, String caller, String msg) { return format.replace("timestamp", timer.getTime()).replace("type", type).replace("caller_class", caller).replace("msg", msg); }
